...
**stress_cpu.sh** — искусственная нагрузка на CPU через `stress-ng`. Без аргументов — интерактивный режим. Параметры: количество ядер (`-c`), процент нагрузки (`-p`), длительность в секундах (`-t`), файл лога (`-l`). При отсутствии `stress-ng` устанавливает его автоматически.
